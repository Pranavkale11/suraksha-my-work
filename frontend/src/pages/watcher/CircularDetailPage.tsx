...
export const CircularDetailPage = () => {
  ...
  const fetchCircular = async () => {
    try {
      const res = await circularsApi.detail(id || '');
      setCircular(res);
    } catch (error) {
      console.error(error);
  ...
  const handleDownload = () => {
    circularsApi.download(id || '');
  };

  const handleReparse = async () => {
    try {
      await circularsApi.reparse(id || '');
      fetchCircular();
    } catch (error) {
      console.error(error);
    }
  ...
  if (!circular) return <div className="min-h-screen bg-slate-50 flex items-center justify-center">Error</div>;
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'fully_parsed') return <span className="flex items-center gap-1 bg-canara-success/10 text-canara-success px-3 py-1 rounded-full text-xs font-bold border border-canara-success/20"><CheckCircle className="w-4 h-4"/> Fully Parsed</span>;
    if (status === 'partially_parsed') return <span className="flex items-center gap-1 bg-[#FF6B35]/10 text-[#FF6B35] px-3 py-1 rounded-full text-xs font-bold border border-[#FF6B35]/20"><AlertTriangle className="w-4 h-4"/> Partially Parsed</span>;
    return <span className="flex items-center gap-1 bg-canara-danger/10 text-canara-danger px-3 py-1 rounded-full text-xs font-bold border border-canara-danger/20"><XCircle className="w-4 h-4"/> Failed</span>;
  };

  // Combine raw text from clauses for the left pane
        ...
        <div className="mb-6">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-3">
              {circular.ingestion_status === 'fully_parsed' && (
                <button 
                  onClick={() => navigate(`/admin/gaps?circular=${circular.circular_id}`)}
                  className="px-4 py-2 bg-yellow-500 text-slate-900 rounded-lg hover:bg-yellow-400 font-bold flex items-center gap-2 shadow-sm"
                >
                  <BrainCircuit className="w-4 h-4" /> Run Gap Detection
                </button>
              )}
              <button onClick={handleDownload} className="px-4 py-2 border border-slate-300 bg-white rounded-lg text-slate-700 hover:bg-slate-50 font-medium flex items-center gap-2 shadow-sm">
                <Download className="w-4 h-4" /> Download
              </button>
              <button onClick={handleReparse} className="px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 font-medium flex items-center gap-2 shadow-sm">
        ...
        <div className="bg-canara-primary/5 border border-canara-primary/20 rounded-xl p-5 mb-8 flex gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              The Watcher module extracted {circular.clauses_extracted} clauses from this {circular.pages_processed}-page document in {circular.processing_time_ms}ms using {circular.parser_version}. 
              All clauses have been embedded and are ready for policy gap detection.
            </p>
          </div>
        </div>
